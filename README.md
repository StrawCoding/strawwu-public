...
公開發行倉庫：StrawWU ISO 版本清單與 GitHub Pages 下載站。
...
ISO 約 4–5 GiB。GitHub Releases 以分片（`.part`）發布（單檔 2 GiB 上限），下載後以 `join-iso.sh` 合併並以 `SHA256SUMS` 驗證。

**禁止**使用 Hermes 建置機、本機 nginx、wastebase 或 apt 鏡像作為 ISO 下載節點。
...
2. 從對應 Release 下載全部 `*.part`、`SHA256SUMS`、`join-iso.sh`
3. 合併並驗證：
